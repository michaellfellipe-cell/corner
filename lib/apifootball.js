...
/** Extrai médias de corners da resposta de team stats */
export function parseCornersAvg(statsRes) {
  if (!Array.isArray(statsRes) || !statsRes[0]) return null;
  const c = statsRes[0].corners;
  if (!c) return null;
  return {
    forHome:     parseFloat(c.for?.average?.home     || 0),
    forAway:     parseFloat(c.for?.average?.away     || 0),
    againstHome: parseFloat(c.against?.average?.home || 0),
    againstAway: parseFloat(c.against?.average?.away || 0),
    form:        statsRes[0].form || "",
    teamId:      statsRes[0].team?.id,
  };
}
...
/** Classifica ofensividade de uma formação */
export function formationAttackScore(formation) {
  if (!formation) return 0;
  const parts = (formation || "4-4-2").split("-");
  const attackers   = parseInt(parts[parts.length - 1]) || 1;
  const midfielders = parseInt(parts[parts.length - 2]) || 4;
  // 4-3-3 → 3att = ofensivo; 5-4-1 → 1att = defensivo
  if (attackers >= 3) return 2;
  if (attackers === 2 && midfielders <= 3) return 1;
