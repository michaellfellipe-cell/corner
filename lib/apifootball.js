...
async function afFetch(path, cacheKey, ttlMs = 60000) {
  ...
  const nullCk   = ck + "__null";  // cache negativo

  const hit = cacheGet(ck);
  if (hit !== null) return hit;

  // Cache negativo: se já falhou recentemente, não tenta de novo
  const nullHit = cacheGet(nullCk);
  if (nullHit !== null) return null;

  try {
    const res = await fetch(`${AF_BASE}${path}`, {
      headers: { "x-apisports-key": apiKey, "Accept": "application/json" },
      signal: AbortSignal.timeout(10000), // 6s → 10s
    });
    if (!res.ok) {
      // Guarda cache negativo 90s para não retentar imediatamente
      cacheSet(nullCk, true, 90_000);
      return null;
    }
    ...
    if (result !== null) {
      cacheSet(ck, result, ttlMs);
    } else {
      cacheSet(nullCk, true, 90_000);
    }
    return result;
  } catch {
    cacheSet(nullCk, true, 60_000); // timeout/network error: retry em 60s
    return null;
  }
}
...
export async function getLiveFixtures() {
  return afFetch("/fixtures?live=all", "af_live_all", 15_000); // v36: 15s
}
...
export async function getTeamCornerHistory(teamId, last = 8) {
  ...
  try {
    // Passo 1 — lista dos últimos jogos finalizados (cache 24h — jogos FT não mudam)
    const fixturesRes = await afFetch(
      `/fixtures?team=${teamId}&last=${last}&status=FT`,
      `af_team_recent_${teamId}_${last}`,
      86_400_000,  // 24h — jogos finalizados não mudam
    );
    ...
    if (!cornerTotals.length) {
      cacheSet(cacheKey, null, 28_800_000);
      return null;
    }
    ...
    cacheSet(cacheKey, result, 28_800_000);
    return result;
  } catch {
    return null;
  }
...
export async function getInjuries(fixtureId) {
  return afFetch(`/injuries?fixture=${fixtureId}`, `af_inj_${fixtureId}`, 28_800_000);
}

// ── Parsers ───────────────────────────────────────────────────────────────────

// ── Dicionário de aliases: apelidos ESPN → nome AF normalizado ───────────────
// Resolve casos onde ESPN usa apelido e AF usa nome oficial (ou vice-versa)
const ALIASES = {
  // Inglaterra
...
/** Aplica aliases a um nome normalizado */
...
export function normName(s) {
  const base = (s || "")
    ...
    .replace(/\b(fc|cf|sc|ac|afc|bfc|sfc|rc|rcd|sd|ud|cd|if|sk|bk|fk|athletic|sporting)\b/g, "")
    .replace(/[^a-z0-9\s]/g, "")
    .replace(/\s+/g, " ")
    .trim();
  return applyAlias(base);
}

/** Tenta casar um jogo ESPN com um fixture da AF (com aliases) */
export function matchAFFixture(afFixtures, espnHome, espnAway) {
  ...
  for (const fix of afFixtures) {
    ...
    if (score > bScore && score >= 0.40) { bScore = score; best = fix; }
  }
  return best;
}
