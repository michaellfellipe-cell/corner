 ...
 *
 * As casas de apostas operam nestas faixas de 10 min:
 *   0-9 | 10-19 | 20-29 | 30-39 | 40-49 | 50-59 | 60-69 | 70-79 | 80-FIM
 *
 * Estratégia: detectar ANTES da virada para entrar na próxima faixa.
 *   Signal em 56-57' → aposta na faixa 60-69
 *   Signal em 66-67' → aposta na faixa 70-79
 *   Signal em 76-77' → aposta na faixa 80-FIM
 *
 * ESTRUTURA DO 2ºT (3 sub-fases):
 *   FASE 2A (45-57'): preparação → alvo faixa 60-69
 *   FASE 2B (58-67'): transição → alvo faixa 70-79
 *   FASE 2C (68-79'): pressão final → alvo faixa 80-FIM
 *
 * 1ºT alinhado às faixas 0-49:
 *   FASE 1A (1-27'):  alvo faixa 30-39 (com Fast Track)
 *   FASE 1B (28-38'): alvo faixa 40-49
 *   FASE 1C (39-44'): transição para intervalo
 */
...
const LATE_START     = 45;
const BLOCK_ALERTS   = 83;   // após 83' bloqueia (80-FIM dá ~7min úteis)
const MAX_MULT_EARLY = 1.35;
const MAX_MULT_LATE  = 1.70;
const LEAGUE_AVG_EARLY = 0.090;
const LEAGUE_AVG_LATE  = 0.130;

// Thresholds baseados em dados reais (62%+ no 2ºT acerta 95%)
...
// Antecipa 2 minutos antes da virada de faixa
function getTargetBetWindow(minute) {
  // Últimos 2 min de cada faixa → aponta para a PRÓXIMA janela
  const decadeRemainder = minute % 10;
  const isPreTransition = decadeRemainder >= 8; // min 8 ou 9 de cada faixa
  ...
  if (isPreTransition && nextDecade <= 90) {
    // Aponta para a PRÓXIMA janela
    targetStart = nextDecade;
    targetEnd   = nextDecade + 9 === 89 ? 90 : nextDecade + 9;
    targetLabel = nextDecade >= 80 ? "80-FIM" : `${nextDecade}-${nextDecade + 9}min`;
    isNext = true;
  } else {
    // Aponta para a janela ATUAL
    targetStart = currentDecade;
    targetEnd   = currentDecade + 9 >= 80 ? 90 : currentDecade + 9;
...
export function analyzeFinalWindow(game) {
  ...
  const totalCorners = H.corners + A.corners;
  const baseRate     = rate(totalCorners, min); // corners/min acumulado

  // ── 1. Estimativa de acréscimos ──────────────────────────────────────────
  // Base: 3 minutos
  // +0.5 por cada gol (paralisação para comemoração/posição)
  // +0.3 por cada cartão amarelo (tempo perdido)
  // Cap: 7 minutos
  const totalGoals   = game.score.home + game.score.away;
  const totalYellows = (game.yellowCards?.home ?? 0) + (game.yellowCards?.away ?? 0);
  const estimatedStoppage = clamp(3 + (totalGoals * 0.5) + (totalYellows * 0.3), 2, 7);

  // Minutos restantes + acréscimos
  const minsFrom90    = Math.max(0, 90 - min);
  const effectiveMins = minsFrom90 + estimatedStoppage;

  // ── 2. Taxa ajustada para a fase final ───────────────────────────────────
  // Combina: taxa acumulada do jogo + pressão da fase final
  const crossRate       = H.crossRate + A.crossRate;
  const blocked         = H.blocked + A.blocked;
  const saves           = H.saves + A.saves;
  const scoreDiff       = Math.abs(game.score.home - game.score.away);
  const losingTeam      = scoreDiff >= 1;
  const tightGame       = scoreDiff <= 1;
  const gameIsSettled   = scoreDiff >= 2; // time líder gerencia o jogo

  // Multiplicador de pressão final (específico para Over 0.5)
  let finalMult = 1.0;
  const reasons = [];

  // Pressão do time perdendo
  if (losingTeam && scoreDiff === 1) {
    finalMult += 0.30;
    reasons.push({ text: "Time perdendo por 1 — busca escanteio desesperadamente", color: "#00e5a0", strong: true });
  } else if (gameIsSettled) {
    finalMult -= 0.30;
    reasons.push({ text: `Jogo definido (${game.score.home}-${game.score.away}) — time líder gerencia`, color: "#ff4560", strong: true });
  }

  // Empate = ambos atacam
  if (scoreDiff === 0) {
    finalMult += 0.20;
    reasons.push({ text: "Empate — ambos pressionam por gol", color: "#00e5a0", strong: true });
  }
  ...
  if (crossRate > 0.22) {
    finalMult += 0.18;
    reasons.push({ text: `Cruzamentos ativos (${(crossRate*10).toFixed(1)}/10min)`, color: "#00e5a0", strong: false });
  } else if (crossRate > 0.14) {
    finalMult += 0.09;
    reasons.push({ text: `Cruzamentos presentes (${(crossRate*10).toFixed(1)}/10min)`, color: "#f0c040", strong: false });
  } else if (crossRate < 0.08 && !losingTeam) {
    finalMult -= 0.15;
    reasons.push({ text: "Poucos cruzamentos — jogo estagnado", color: "#ff4560", strong: false });
  }

  // Goleiros exigidos (pressão real)
  if (saves >= 4) {
    finalMult += 0.12;
    reasons.push({ text: `Goleiros muito exigidos (${saves} defesas)`, color: "#00e5a0", strong: false });
  } else if (saves >= 2) {
    finalMult += 0.06;
    reasons.push({ text: `Goleiros ativos (${saves} defesas)`, color: "#f0c040", strong: false });
  }

  // Chutes bloqueados perto da área
  if (blocked >= 4) {
    finalMult += 0.10;
    reasons.push({ text: `Muitos chutes bloqueados (${blocked})`, color: "#00e5a0", strong: false });
  }

  // Ritmo de corners do jogo: se já é alto, continua
  const cornerRate80 = rate(totalCorners, min);
  if (cornerRate80 > 0.20) {
    finalMult += 0.15;
    reasons.push({ text: `Ritmo alto de escanteios (${(cornerRate80*10).toFixed(1)}/10min histórico)`, color: "#00e5a0", strong: false });
  } else if (cornerRate80 > 0.14) {
    finalMult += 0.08;
    reasons.push({ text: `Ritmo moderado de escanteios`, color: "#f0c040", strong: false });
  } else if (cornerRate80 < 0.08) {
    finalMult -= 0.12;
    reasons.push({ text: "Jogo com poucos escanteios historicamente", color: "#ff4560", strong: false });
  }

  // ── 3. Lambda para Poisson ────────────────────────────────────────────────
  // Taxa base do jogo ajustada pelo multiplicador final
  // No 80'+, o ritmo tende a AUMENTAR (pressão), então usa 1.3× da taxa
  const finalRate = clamp(baseRate * finalMult * 1.3, 0.05, 0.45);
  const lambda    = finalRate * effectiveMins;

  // ── 4. Probabilidade Poisson ──────────────────────────────────────────────
  // P(≥1 corner) = 1 - P(0 corners) = 1 - e^(-λ)
  const probOver05  = 1 - Math.exp(-lambda);
  const probOver15  = 1 - Math.exp(-lambda) - lambda * Math.exp(-lambda);
  const projCorners = +lambda.toFixed(1);

  // ── 5. Veredito ───────────────────────────────────────────────────────────
  let verdict, verdictColor, verdictIcon;
  if (gameIsSettled && crossRate < 0.12) {
    verdict      = "EVITAR — Jogo gerenciado";
    verdictColor = "#ff4560";
    verdictIcon  = "❌";
  } else if (probOver05 >= 0.88) {
    verdict      = "MUITO VIÁVEL — Over 0.5";
    verdictColor = "#00e5a0";
    verdictIcon  = "✅";
  } else if (probOver05 >= 0.75) {
    verdict      = "VIÁVEL — Over 0.5";
    verdictColor = "#00e5a0";
    verdictIcon  = "✅";
  } else if (probOver05 >= 0.60) {
    verdict      = "MODERADO — Avaliar contexto";
    verdictColor = "#f0c040";
    verdictIcon  = "⚠️";
  } else {
    verdict      = "BAIXA VIABILIDADE";
    verdictColor = "#ff4560";
    verdictIcon  = "❌";
  }
  ...
  return {
    ...
    lambda:            +lambda.toFixed(2),
    probOver05:        Math.round(probOver05 * 100),
    probOver15:        Math.round(Math.max(0, probOver15) * 100),
    projCorners,
    finalRate:         +finalRate.toFixed(3),
    finalMult:         +clamp(finalMult, 0.4, 2.0).toFixed(2),
    reasons,
  ...
  else if (minsLeft > 25) { label = `~${minsLeft} minutos disponíveis`;  urgency = "good";    }
  else if (minsLeft > 12) { label = `~${minsLeft} min — entrar agora`;   urgency = "warning"; }
...
export function projectCorners(game) {
  ...
  const confidence  = calcConfidence(projected10, factors, mult, game, baseRate, isEarly, isFastTrack, isPostGoalCooldown);
  ...
  if (isEarly) {
    ...
    subPhase = min < 58 ? "2A" : min < 68 ? "2B" : "2C";
  }
