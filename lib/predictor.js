...
function calcCornerMomentum(leagueAfId, baseRate, isEarly) {
  ...
  if (ratio >= 1.4) return { delta: 0.06, label: `Ritmo acima da média da liga (${ratio.toFixed(1)}×)`, impact: "medium" };
  if (ratio <= 0.5 && baseRate > 0) return { delta: -0.06, label: `Ritmo abaixo da média da liga (${ratio.toFixed(1)}×)`, impact: "low" };
  return { delta: 0, label: null };
}
