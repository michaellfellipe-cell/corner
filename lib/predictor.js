...
// Substitui 0.090/0.130 fixo por médias reais do confronto
function calcHistoricalLeagueAvg(game, isEarly) {
  const h = game.historical;
  if (!h) return null;

  // Home team playing at home: corners FOR home + corners AGAINST away
  const homeExp = (h.homeCornerAvgHome || 0) + (h.awayCornerAgstAway || 0);
  // Away team playing away: corners FOR away + corners AGAINST home
  const awayExp = (h.awayCornerAvgAway || 0) + (h.homeCornerAgstHome || 0);

  // Média ponderada: 60% stats individuais + 40% H2H se disponível
  let expected = (homeExp + awayExp) / 2;

  if (h.h2hEstCorners && h.h2hGames >= 3) {
    expected = expected * 0.60 + h.h2hEstCorners * 0.40;
  }

  // Limita para faixa realista: 4–16 corners/jogo
  expected = clamp(expected, 4, 16);

  // Divide por 90 para obter corners/min
  // No 1ºT, usa ~45% do total esperado (corners são mais raros no começo)
  return isEarly
    ? (expected * 0.45) / 45  // 45% dos corners no 1ºT dividido pelos 45 minutos
    : (expected * 0.55) / 45; // 55% no 2ºT
}
