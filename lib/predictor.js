...
// ── Detector de gol recente (qualquer tempo) ──────────────────────────────────
function detectRecentGoal(game) {
  if (!Array.isArray(game.goalEvents) || !game.goalEvents.length) return false;
  const min = game.minute;
  return game.goalEvents.some(e => {
...
function calcCornerMomentum(leagueAfId, baseRate, isEarly) {
  ...
  if (ratio >= 1.25) return { delta: 0.06, label: `Ritmo acima da média da liga (${ratio.toFixed(1)}×)`, impact: "medium" };
...
function detectPostGoalCooldown(game, H, A) {
  ...
  // Só inclui cruzamentos na análise se o dado está disponível
  const crossMissing = isCrossDataMissing(game, H, A);
  if (crossMissing) {
    // Usa só chutes como proxy
...
function calcEarlyPressure(H, A, game) {
  ...
  // 6. Domínio territorial — só conta quando acompanhado de pressão real
  // Posse isolada não gera corner; posse + ação na área sim
  const maxPoss = Math.max(H.possession, A.possession);
  const totalShots2 = H.shots + A.shots;
  const totalCross2 = H.crosses + A.crosses;
  if (maxPoss >= 68 && (totalShots2 >= 6 || totalCross2 >= 3)) {
...
function calcLatePressure(H, A, game) {
  ...
  // antes de 52': dados ainda diluídos pelo 1ºT
  const totalCorners2 = H.corners + A.corners;
  const cRate2 = rate(totalCorners2, min);
  if (phase2A && min >= 52) {
    // Fase 2A avançada: threshold mais alto para evitar falsos positivos
    if (cRate2 > 0.22)      add("Ritmo alto de escanteios no início do 2ºT", `${(cRate2*10).toFixed(1)}/10min`, "medium", 0.09);
    else if (cRate2 > 0.16) add("Ritmo moderado de escanteios",              `${(cRate2*10).toFixed(1)}/10min`, "low",    0.05);
  } else if (!phase2A) {
    const cThresh = phase2B ? 0.18 : 0.22;
    if (cRate2 > cThresh + 0.06)  add("Ritmo alto de escanteios",            `${(cRate2*10).toFixed(1)}/10min`, "high",   0.14);
    else if (cRate2 > cThresh)    add("Ritmo moderado de escanteios",        `${(cRate2*10).toFixed(1)}/10min`, "medium", 0.07);
  }
...
export function projectCorners(game) {
  ...
  const momentumFactor = calcCornerMomentum(game.leagueAfId, baseRate, isEarly);
...
function calcConfidence(proj10, factors, mult, game, baseRate, isEarly, isFastTrack, isPostGoalCooldown) {
  ...
  if (isEarly) {
    ...
    // Fase 2A (45-57'): penalidade suave — dados do início do 2ºT
    // Reduzida de 0.90 → 0.93: aos 52'+, há dados suficientes para análise
    if (game.minute < 52) conf *= 0.90;
    else if (game.minute < 58) conf *= 0.93;

    // Gol recente no 2ºT = reorganização em andamento (5 min)
    if (detectRecentGoal(game)) conf *= 0.75;
