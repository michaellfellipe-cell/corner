...
export function projectCorners(game) {
  ...
  const totalCorners = H.corners + A.corners;
  // FIX D: no 2ºT, usar taxa baseada nos minutos do 2ºT (mais preditivo)
  // corners acumulados no 1ºT diluem o baseRate se usado com min total
  // Estimativa: metade dos corners geralmente acontece em cada tempo
  let baseRate = rate(totalCorners, min);
  if (!isEarly && min > 50) {
    // Estima corners do 2ºT: total - estimativa_1T (média: 44% dos corners no 1ºT)
    const estimatedHT = totalCorners * 0.44;
    const cornersIn2T = Math.max(0, totalCorners - estimatedHT);
    const minsIn2T    = Math.max(1, min - 45);
    const baseRate2T  = rate(cornersIn2T, minsIn2T);
    // Usa média ponderada: 60% taxa atual, 40% taxa do 2ºT (mais reativo)
    baseRate = baseRate * 0.60 + baseRate2T * 0.40;
  }
...
function calcConfidence(proj10, factors, mult, game, baseRate, isEarly, isFastTrack, isPostGoalCooldown) {
  ...
  if (isEarly) {
    ...
    const finalPenalty = Math.min(0.97,
      basePenalty + evidenceBoost + (1 - basePenalty) * (1 - multFactor)
    );
    conf *= finalPenalty;
    ...
    // Fase 2A (45-57'): penalidade baseada em evidências concretas
    // Os corners acumulados no 1ºT são FATOS — reduzem a incerteza do início do 2ºT
    if (game.minute < 58) {
      ...
      const leagueAvgLate     = getLeagueAvg(game.leagueAfId, false);
      const cornerRatioLate   = leagueAvgLate > 0 ? baseRate / leagueAvgLate : 1;
      const ratioBonusLate    = cornerRatioLate >= 3.0 ? 5
                              : cornerRatioLate >= 2.0 ? 3
                              : cornerRatioLate >= 1.5 ? 1 : 0;

      const evidenceLate = Math.min(10,
        totalCornersLate  * 2.0 +
        totalBlockedLate  * 1.5 +
        totalOnTargetLate * 1.0 +
        ratioBonusLate
      );
      const evBoostLate = evidenceLate * 0.025;

      // Base penalty: 0.90 antes de 52', 0.93 depois — evidência reduz ambos
      const basePenLate = game.minute < 52 ? 0.90 : 0.93;
      const mfLate = 1 / (1 + (mult - 1) * 2);
      const finalPenLate = Math.min(0.99,
