...
function calcLatePressure(H, A, game) {
  ...
  if (phase2A && min >= 52) {
    ...
    } else if (cRate2 > 0.13 && crossMissing2) {
      // FIX B: em ligas sem crosses, taxa de corners é proxy direto de pressão
      add("Taxa de escanteios elevada (liga sem cruzamentos)", `${(cRate2*10).toFixed(1)}/10min`, "medium", 0.09);
    } else if (cRate2 > 0.10 && crossMissing2) {
...
function calcConfidence(proj10, factors, mult, game, baseRate, isEarly, isFastTrack, isPostGoalCooldown) {
  ...
  if (isEarly) {
    ...
    const evidenceBoost = evidenceScore * 0.025; // +2.5% por ponto, max +25%

    // Penalidade base por fase (quanto mais tarde, menor)
    let basePenalty;
    if      (game.minute < 10) basePenalty = 0.45; // P0: muito cedo
    else if (game.minute < 20) basePenalty = 0.65; // P1
    else if (game.minute < 30) basePenalty = 0.78; // P2
    else if (game.minute < 38) basePenalty = 0.88; // P3
