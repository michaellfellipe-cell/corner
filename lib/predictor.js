...
/**
 * Detecta se o dado de cruzamentos está ausente (ESPN não cobre todas as ligas).
 * Critérios: crosses = 0 para AMBOS os times E o jogo tem atividade real
 * (corners, chutes, passes) — evidência de que o jogo acontece mas a ESPN
 * simplesmente não reporta cruzamentos para essa liga.
 */
...
function calcEarlyPressure(H, A, game) {
  ...
  // SKIP se dado ausente (ESPN não cobre esta liga)
...
function calcLatePressure(H, A, game) {
  ...
  // SKIP se dado ausente — ESPN não reporta cruzamentos para todas as ligas
  const crossRate = H.crossRate + A.crossRate;
  const crossMissing = isCrossDataMissing(game, H, A);
  if (!crossMissing) {
    if (phase2A) {
...
export function analyzeFinalWindow(game) {
  ...
  // Se ESPN não fornece cruzamentos para esta liga, usa corners e shots inside box como proxy
  const crossMissing = isCrossDataMissing(game, H, A);
  if (crossMissing) {
    ...
    if (sibRate > 0.15 || baseRate > 0.15) {
      finalMult += 0.10;
      reasons.push({ text: "Pressão na área confirmada (cruzamentos ESPN indisponível)", color: "#f0c040", strong: false });
    } else {
...
export function projectCorners(game) {
  ...
  // 6. NOVO: Offsides — linhas ofensivas avançadas
...
function calcConfidence(proj10, factors, mult, game, baseRate, isEarly, isFastTrack, isPostGoalCooldown) {
  ...
  if (isEarly) {
    ...
    // (ESPN não reporta cruzamentos para todas as ligas)
    ...
    // NOVO: gol recente no 2ºT = reorganização em andamento (5 min)
